---
name: "🔬 Research Intake Submission"
about: "Submit a research paper, article, thread, or theory for Shutri AI + Human peer review and SMS media publication."
title: "[INTAKE] Research Submission: <Research Title>"
labels: "mempool, research-intake"
assignees: "ashutoshmjain"
---

## 📌 Research Overview
- **Research Title:** 
- **Primary Authors / Sources:** 
- **Source URL(s):** 
- **Code / Supplemental Repo (Optional):** 

---

## 🎯 Core Thesis & Key Insights
Provide a concise summary of the core thesis, key findings, and theoretical framework.
- **Thesis Statement:** 
- **Key Concepts:** 
  1. 
  2. 
  3. 

---

## 🧮 Mathematical & Technical Rigor (KaTeX)
List any mathematical proofs, formulas, or formal definitions that require KaTeX hardening ($inline$ or $$display$$).
- **Key Equations / Formulas:**

---

## 🎧 Media & Syndication Targets
- [x] **Text Article:** `deepDive/src/` (mdBook PWA Ledger)
- [x] **Audio Podcast:** NotebookLM / Spotify / Apple / Fountain
- [x] **Square Video (740x740):** DDMA / YouTube / Instagram / TikTok / Nostr (<20MB)

---

## 🛡️ License & Safety Self-Declaration
- [x] I confirm that this research contains NO private, confidential, or copyrighted material.
- [x] I agree that all review notes, audio overviews, and visual assets will be published under **CC BY-SA 4.0**.
